...
class Context(object):
    ...
    def __init__(self):
        ...
        self.node_list = []
        self.user_list = []
        self.current_user = None # if overriden by '-N'
        self.unicast = None
        self.multicast = None
        self.admin_ip = None
...
def init_cluster_local():
    ...
    #TODO! Do we still need the hacluster? Maybe for the hawk?
...
def append_unique(fromfile, tofile, user, remote=None, from_local=False):
    ...
    if not utils.check_file_content_included(
                fromfile, tofile, remote=remote, source_local=from_local):
        if from_local and remote:
...
def append_unique_text(text, tofile, user, remote=None, from_local=False):
    ...
    if not remote:
        append fromfile to tofile, locally
    """
    if not utils.check_text_included(text, tofile, remote, from_local):
        if from_local and remote:
...
def configure_ssh_key(user, remote=None):
    ...
    if remote is None:
        ...
        authorized_file = "/home/" + user + "/.ssh/authorized_keys"
...
def swap_public_ssh_key(local_user, remote_user, remote_node, add=False):
    ...
    if add:
        configure_ssh_key(remote_user, remote_node)
...
def join_csync2(seed_host):
    ...
    with logger_utils.status_long("Configuring csync2"):
        ...
        rc, _, err = invoke("scp %s@%s:'/etc/csync2/{csync2.cfg,key_hagroup}' /etc/csync2" % (remote_user, seed_host))
        if not rc:
        ...
        if not invokerc('ssh {} {}@{} "csync2 -rm /; csync2 -rxv || csync2 -rf / && csync2 -rxv"'.format(SSH_OPTION, remote_user, seed_host)):
            print("")
...
def setup_passwordless_with_other_nodes(init_node):
    ...
    for node in cluster_nodes_list:
        swap_public_ssh_key(local_user, utils.user_of(node), node)
...
def remove_node_from_cluster():
    ...
    remote_user = _context.current_user
    rc, _, err = invoke('ssh {} {}@{} "bash -c \\\"rm -f {}\\\""'.format(
            SSH_OPTION, remote_user, node, " ".join(_context.rm_list)))
    if not rc:
...
""")
        ...
        while n < REJOIN_COUNT:
            if utils.service_is_active("pacemaker.service", remote_user=utils.user_of(cluster_node), remote_addr=cluster_node):
                break
            n += 1
